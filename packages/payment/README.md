...
| **`returnURL`**                             | <code>string</code>                                                                                     | Custom URL scheme used to return to the app after redirect-based authentication. iOS only. Stripe does not offer otherwise eligible redirect-based payment methods when this is not configured. Forward the returned URL to `handleURLCallback` from the app URL handler. | <code>""</code>          | 3.0.0 |
